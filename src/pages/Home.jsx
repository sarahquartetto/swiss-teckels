...
import AddTeckelModal from '@/components/teckels/AddTeckelModal';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-stone-950 via-stone-900 to-stone-950">
      ...
      <motion.div
        ...
        className="relative text-left overflow-hidden min-h-[480px] h-[72vh] flex items-center justify-start sm:min-h-[560px] sm:h-[78vh] lg:min-h-[680px] lg:h-[82vh]"
      >
          {/* Hero background image (place file in `public/teckel-hero.jpg`) */}
          <div
            className="absolute inset-0 bg-top bg-cover bg-no-repeat opacity-60 [background-image:url('/teckel-hero.png')]"
            aria-hidden="true"
          />
          {/* Contrast overlay */}
          <div
            className="absolute inset-0 bg-gradient-to-b from-stone-950/15 via-stone-950/45 to-stone-950/70"
            aria-hidden="true"
          />

          <div className="relative w-full max-w-7xl mx-auto px-4 py-12 sm:px-6 sm:py-16">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 py-8 sm:py-12">
        ...
        {isError && (
          <div className="mb-10 rounded-2xl border border-red-900/30 bg-red-950/20 p-5 text-red-200">
            <p className="font-semibold mb-1">Erreur de connexion à la base de données</p>
            <p className="text-sm text-red-200/80 break-words">{String(error?.message || error)}</p>
            <p className="text-xs text-red-200/70 mt-3">
              Vérifie que l’API tourne (`cd server && npm run dev`) et que `server/.env` est correct.
            </p>
          </div>
        )}
        ...
        >
          <div className="flex flex-col sm:flex-row items-stretch justify-center gap-3">
            ...
            <Button
              ...
              className="h-[52px] px-5 rounded-xl bg-gradient-to-r from-amber-700 to-amber-900 text-amber-50 hover:from-amber-600 hover:to-amber-800"
            >
        ...
        <AddTeckelModal open={addOpen} onClose={() => setAddOpen(false)} />
